...
Storage.prototype.add_package = function(name, metadata, callback) {
  ...
  function check_package_remote(cb) {
	if (self.config.localFirst) {
		console.log('storage.js check_package_remote(): config.localFirst = true, will not sync up with remote')
		return cb();
	}
  ...
  rstream.on('error', function(err) {
    ...
    self.local.get_package(name, function(err, info) {
      if (!err && info._distfiles && info._distfiles[filename] != null) {
		...
		console.log('storage.js  get_tarball() sync remote!!!');
...
Storage.prototype.get_package = function(name, options, callback) {
  ...
  // self.config.localFirst=true;
	if (self.config.localFirst && data) {
		console.log('storage.js  get_package(): "config.localFirst" is true, will not sync remote');
